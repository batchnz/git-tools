 ...
 */
async function init() {
  process.stdout.write(colors.green(`Uninstalling development dependencies... `));

  // 1. Install dev dependencies in parent package.json
  const { err } = await exec(`cd ${initPath} && npm r ${devDependencies.join(' ')} -D`);
  ...
  configFiles.forEach((filename) => {
    fs.unlink(`${initPath}/${filename}`, (err) => {
      if (err) {
        console.log(colors.red(`error: ${err}`));
        return;
      }
      console.log(colors.yellow(`${filename} was removed.`));
    });
  });
}
